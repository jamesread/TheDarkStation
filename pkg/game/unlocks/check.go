...
func IsDeckTravelUnlocked(p RunProgress, deckID int) bool {
	...
	// Sequential progression: deck N requires deck N-1 to be reachable first.
	if deckID > 0 && !IsDeckTravelUnlocked(p, deckID-1) {
		return false
	}
...
func DeckTravelBlockReason(p RunProgress, deckID int) string {
	...
	if deckID > 0 && !IsDeckTravelUnlocked(p, deckID-1) {
		return fmt.Sprintf("Needs: deck %d unlocked first", deckID)
	}
	if p.LiftRoutingPowered == nil || !p.LiftRoutingPowered[deckID] {
