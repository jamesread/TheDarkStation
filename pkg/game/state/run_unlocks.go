...
// IsRunWideKeycardName reports whether an item name is a run-persistent keycard.
func IsRunWideKeycardName(name string) bool {
	if name == "" {
		return false
	}
	...
	return strings.HasPrefix(name, "Reactor Authorization")
}
...
// PromoteOwnedRunKeycards moves run-wide keycards from deck inventory into RunInventory.
func (g *Game) PromoteOwnedRunKeycards() {
	if g == nil {
		return
	}
	var names []string
	g.OwnedItems.Each(func(item *world.Item) {
		...
		if IsRunWideKeycardName(item.Name) && !g.HasRunKeycard(item.Name) {
	...
	for _, name := range names {
		...
		g.AddRunKeycard(world.NewItem(name))
	}
}
