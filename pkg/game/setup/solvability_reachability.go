...
func CanEnterCellAtInit(g *state.Game, cell *world.Cell) (bool, MovementBlockReason) {
	...
	if gameworld.HasGenerator(cell) || gameworld.FurnitureBlocksMovement(cell) ||
		gameworld.HasTerminal(cell) || gameworld.HasPuzzle(cell) ||
		gameworld.HasMaintenanceTerminal(cell) || gameworld.HasHazardControl(cell) ||
		gameworld.RepairDeviceBlocksMovement(cell) || gameworld.HasBlockingRepairBlocker(cell) ||
...
// InitialReachableCells returns cells reachable from the player entry at level init.
func InitialReachableCells(g *state.Game) *mapset.Set[*world.Cell] {
	...
	for _, entry := range entryDoorPowerSeeds(g) {
		bfsInitReachable(g, entry, &reachable, nil)
	}
	return &reachable
}

func bfsInitReachable(g *state.Game, entry *world.Cell, reachable *mapset.Set[*world.Cell], locked *mapset.Set[*world.Cell]) {
	if g == nil || entry == nil || reachable == nil {
		return
	}
...
func InitialReachableWithLockedDoors(g *state.Game, locked *mapset.Set[*world.Cell]) *mapset.Set[*world.Cell] {
	...
	reachable := mapset.New[*world.Cell]()
	for _, entry := range entryDoorPowerSeeds(g) {
		bfsInitReachable(g, entry, &reachable, locked)
	}
	return &reachable
}
