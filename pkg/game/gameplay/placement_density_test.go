...
func TestSetupLevel_mapTxtSeed_hasGeneratorsAndFurniture(t *testing.T) {
	...
	if gen != wantGens {
		t.Fatalf("expected %d generators on level %d map.txt seed, got %d", wantGens, g.Level, gen)
	}
...
func TestSetupLevel_level5Seeds_haveGeneratorsAndFurniture(t *testing.T) {
	...
	if bare > 0 {
		t.Fatalf("%d/20 level-5 seeds still missing generators or furniture", bare)
	}
}
